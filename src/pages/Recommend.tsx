...
export default function Recommend() {
  ...
  return (
    <div className="container mx-auto px-4 py-6 sm:p-6 md:p-10 max-w-4xl">
      <div className="mb-6 sm:mb-8">
        <div className="text-xs uppercase tracking-widest text-primary">Smart recommendations</div>
        <h1 className="font-display text-3xl sm:text-4xl md:text-5xl mt-2 leading-tight">What you should do next.</h1>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5 sm:mb-6 surface-card rounded-2xl p-3 sm:p-4 border border-border/60">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          <Layers className="size-4 text-primary shrink-0" />
          <span className="text-xs sm:text-sm font-medium truncate">3 Choices Only Mode</span>
          <Switch
        ...
        <Button variant="ghost" size="sm" onClick={() => { setSeed((s) => s + 1); store.incDecisionsAvoided(1); }}>
          <RefreshCw className="sm:mr-2 size-4" /> <span className="hidden sm:inline">Refresh</span>
        </Button>
      </div>

      {recs.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-border p-8 sm:p-12 text-center text-muted-foreground">
        ...
        <div className="space-y-3">
          {recs.map(({ task, reason, score }, i) => (
            <div
              key={task.id}
              className="surface-card rounded-3xl p-4 sm:p-5 border border-border/60 hover:-translate-y-0.5 hover:shadow-elegant transition-smooth animate-slide-in-right"
              style={{ animationDelay: `${i * 60}ms` }}
            >
              <div className="flex items-start gap-3 sm:gap-4">
                <div className="size-10 sm:size-12 rounded-2xl bg-gradient-hero text-primary-foreground flex items-center justify-center font-display text-lg sm:text-xl shadow-glow shrink-0">
                  {i + 1}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="font-display text-lg sm:text-xl break-words">{task.title}</div>
                  ...
                  <div className="mt-3 inline-flex items-start gap-2 text-xs sm:text-sm bg-primary/5 text-foreground rounded-xl px-3 py-2 border border-primary/10">
